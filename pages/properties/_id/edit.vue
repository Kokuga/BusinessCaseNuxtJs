			...
			<form v-on:submit.prevent="onSubmit()"
				...
				<div class="text-center xl:mt-0 mt-8">
					...
					<nuxt-link class="mt-4 px-5 py-3 bg-gray-200 rounded-lg border border-gray-600 text-xl font-semibold"
							   :to="{ name: 'properties-id', params: {id : this.$route.params.id} }">Retour
					</nuxt-link>
				</div>
			</form>
		</div>
	...
	methods: {
		async getData() {
			const data = await this.$axios.get(
				`/api/annonces/${this.$route.params.id}`, {
					'headers': {
						'Authorization': localStorage.getItem('authToken')
					}
				}
				...
				.then((res) => {
					...
					this.dateCar = res['data'].anneeCirculation;
					this.marqueForm = res['data']['Modele']['marque']['id'];
					this.modeleForm = res['data']['Modele']['id'];
					this.carburantForm = res['data']['Carburant']['id'];
				})
		},
		...
		async onSubmit() {
			await this.$axios.post('/annonce/edit', {
				title: this.title,
				price: this.price,
				...
				.then(
					response => {
						this.$router.push({name: "properties-id", params:{id: this.$route.params.id}});
					},
					response => {
						this.error = true
					}
